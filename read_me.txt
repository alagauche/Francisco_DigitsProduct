...
Github:         https://github.com/alagauche/Francisco_DigitsProduct.git

Instructions:
You can just run the code in Francisco_DigitsProduct.py, it will 
spit out the test cases to console.
...
(2) "factor sieve" method
    ...
    - the idea is that if you create the result using the sieve in this 
    "biggest factor first" way, then you will minimize the left-most place of 
    the result, which will result in the lowest positive integer    
(3) while product != 1 loop
    - in this loop, the idea is that we need to loop through the product and 
    use the factor sieve each time until we reach the stop condition
